...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(process.env.NEXT_PUBLIC_API_URL + "/auth/forgot-password" || "http://127.0.0.1:8000/auth/forgot-password", {
        method: "POST",
      ...
      setStatus("success");
      setMessage("Si cette adresse email existe dans notre base de données, un lien de réinitialisation vient de vous être envoyé.");
    } catch (err: unknown) {
      setStatus("error");
