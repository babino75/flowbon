                  ...
                  {filtered.map((item, idx) => (
                    <tr key={`${item.account_id}-${item.category_id || idx}`} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${item.is_active ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-slate-100 text-slate-500 border border-slate-200"}`}>
