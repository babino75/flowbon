...
export default function AdvancesPage() {
  ...
  const fetchCompany = async () => {
    try {
      const [comp, activeFY] = await Promise.all([
        api.getCompany(),
        api.getActiveFiscalYear().catch(() => null)
      ]);
      
      if (comp && (comp as any).currency) {
        setCompanyCurrency((comp as any).currency);
      }
      ...
      console.error("Impossible de récupérer les infos de l'entreprise", err);
    }
  };
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className={`inline-flex items-center justify-center px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition-all hover:scale-[1.02] active:scale-[0.98] ${
                activeFiscalYear 
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-rose-600/80 leading-relaxed">
                  Il est impossible de créer une demande d'avance sans un exercice comptable ouvert pour la période actuelle.
                </p>
              </div>
            </div>
