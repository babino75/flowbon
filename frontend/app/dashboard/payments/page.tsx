...
export default function PaymentsPage() {
  ...
  const fetchPayments = async () => {
    ...
    setError(null);
    try {
      const data = await api.getExpenses({ status: "approved" });
      setExpenses(data as any[]);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Impossible de charger les bons approuvés.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">Bons de dépenses approuvés, en attente de paiement.</p>
          </div>
          <button
            type="button"
