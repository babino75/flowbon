...
def migrate_cash_transactions_to_treasury(migration_map):
    ...
    try:
        ...
        for ct in cash_transactions:
            ...
            existing_tx = db.query(TreasuryTransaction).filter(
                TreasuryTransaction.treasury_account_id == UUID(treasury_account_id),
                TreasuryTransaction.created_at == ct.created_at,
                TreasuryTransaction.amount == ct.amount
            ).first()
            ...
            # Get creator
            creator_id = ct.created_by or db.query(User).filter(
                User.company_id == ct.company_id
            ).first().id
